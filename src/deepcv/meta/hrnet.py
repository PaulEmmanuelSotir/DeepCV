...
import deepcv.utils
from deepcv.utils import NL, NUMBER_T
from .nn import conv_nd, layer, interpolate, get_padding_from_kernel, NormTechnique
from .types_aliases import SIZE_OR_SEQ_OF_SIZE_T, TENSOR_OR_SEQ_OF_TENSORS_T, NORM_TECHNIQUES_MODULES_T

__all__ = ['ParallelConvolution', 'MultiresolutionFusion', 'HRNetV1RepresentationHead', 'HRNetV2RepresentationHead', 'HRNetV2pRepresentationHead', 'hrnet_input_stem']
...
class ParallelConvolution(torch.nn.Module):
    ...
    # TODO: Asuchronously yield each output for each branch? (but how to check tensor count of a generator?)
    # TODO: Allow automatic group calculation like in PyConv so that flops count stay the same even with varying kernel sizes (compared to a regular conv with the smallest kernel_size)
    """

    def __init__(self, input_shapes: SIZE_OR_SEQ_OF_SIZE_T, submodule_params: Dict[str, Tuple[Sequence[Any], Any]], act_fn: Union[Sequence[Optional[Type]], Optional[Type]] = torch.nn.ReLU,
                 preactivation: Union[Sequence[bool], bool] = False, dropout_prob: float = None,
                 batch_norm=None, layer_norm=None, instance_norm=None, group_norm=None, layer_nrm_and_mean_batch_nrm=None, channel_dim: int = 1, raise_on_not_enough_in_tensors: bool = True):
        ...
        This `torch.nn.Module` operation performs a grouped conv operation where each groups are regular/independant convolutions which may be applied on different feature map resolutions (like HRNet parallel streams/branches) and/or have different conv parameters, like different kernel sizes for each group (like PyConv does).
        Args:
            - input_shapes: Input tensors shape(s), with channel dim located at `channel_dim`th dim and with minibatch dimension. These tensor shapes should be shapes on which each parrallel/multi-res convolutions will be performed. This argument can also be used by eventual normalization technique(s) (`input_shapes[channel_dim:]` passed to `deepcv.meta.nn.normalization_techniques`).
            - submodule_params: Keyword arguments dict passed `deepcv.meta.nn.conv_nd`. Each argument can either be a sequence (List, Tuple, ...) of the same lenght as `input_shapes` or a single value depending on if each multires-group convs have differents arg values or share the same argument value.
            ...
            - channel_dim: Channel dim index in provided `input_shapes` tensor shapes. Dimensions after channel dim should be spatial features dims on which convolution is applied (e.g. two dimensions after channel dim for a 2D convoltion)
            - raise_on_not_enough_in_tensors: Boolean indicating whether to raise an exception if too few input tensors are given during forward passes (less than `len(input_shapes)` input tensors given to forward method)

        NOTE: `kernel_size`, `padding` and `dilation` args in `submodule_params` must allways be specified as a sequence of integers (one for each dims, e.g. `kernel_size` cant be a single int, like `2` for a `2x2` kernel, otherwise, there could be ambiguities as conv args can be specified once or as a sequence of values for each parrallel conv groups/branches). I.e., their value must match this type: `Union[Sequence[Sequence[int]], Sequence[int]]` where inner-most sequence being of lenght `dims` and optional parent sequence being of the smae lenght as `input_shapes` sequence. See also related error message in function code.
        NOTE: If `padding` isn't excpilitly provided in `submodule_params` (i.e. in subm YAML spec), padding values are proceessed from each kernel sizes in order to obtain unchanged output feature map sizes (if possible) (See `deepcv.meta.nn.get_padding_from_kernel`).
        ...
        """
        super().__init__()
        if isinstance(input_shapes[0], NUMBER_T):
            input_shapes = [input_shapes, ]
        elif len(input_shapes) == 0:
            raise ValueError('Error in `deepcv.meta.nn.parallel_convolution`, `input_shapes` argument must be a sequence of `torch.Size` with at least one input tensor shape(s). '
                             f'NOTE: If you are performing a single conv on a single tensor shape/resolution, use `deepcv.meta.nn.layer` instead. Got `input_shapes="{input_shapes}"`')
        self.raise_on_not_enough_in_tensors = raise_on_not_enough_in_tensors
        self.channel_dim = channel_dim
        self.spatial_dims = len(input_shapes[0][channel_dim+1:])
        ...
        if not isinstance(act_fn, Sequence):
            act_fn = [act_fn, ] * len(input_shapes)
        if not isinstance(preactivation, Sequence):
            preactivation = [preactivation, ] * len(input_shapes)
        # If act_fn and/or preactivation where sequences, those should have the same lenght as `input_shapes` (one different value for each parallel convolution)
        if any([not len(input_shapes) == len(seq) for seq in {act_fn, preactivation}]):
            raise ValueError('Error: If `preactivation` and/or `act_fn` args are sequences, those should have the same lenght as `input_shapes` (one value for each parallel convolution)')

        # Forbid to specify a single int instead of a sequence of `dims` ints. Nescessary to avoid ambiguities between a sequence of different args for each group/parallel convs or a sequence of ints for each feature map dim (would be ambiguous when `dims==len(input_shapes)`)
        ...
        for i in range(len(input_shapes)):
            ...
            for n, v in submodule_params.items():
                if n in conv_args_with_sequence_constraint:
                    if isinstance(v, Sequence) and len(v) == len(input_shapes) and all([isinstance(sub_v, Sequence) for sub_v in v]):
                        v = v[i]
                    elif not isinstance(v, Sequence) or len(v) not in {1, self.spatial_dims} or any([isinstance(sub_v, Sequence) for sub_v in v]):
                        raise ValueError(f'Error: `{n}` entry of `submodule_params` should either be a different sequence for each group/parallel conv (sequence of sequence of size `len(input_shapes) x [dims={self.spatial_dims} or 1]`) or a single sequence of size `1` or `dims={self.spatial_dims}` (then the value is common to all group/parallel convs).{NL}'
                    ...
                    ith_conv_kwargs[n] = v[i] if isinstance(v, Sequence) and len(v) == len(input_shapes) else v
            all_conv_ops_kwargs.append(ith_conv_kwargs)

        self.group_convolutions = list()
        for i, (shape, ith_conv_kwargs, act, preact) in enumerate(zip(input_shapes, all_conv_ops_kwargs, act_fn, preactivation)):
            if 'padding' not in ith_conv_kwargs:
                ith_conv_kwargs['padding'] = get_padding_from_kernel(ith_conv_kwargs['kernel_size'], warn_on_uneven_kernel=False)
            conv_op = conv_nd(**ith_conv_kwargs)
            layer_op = layer(layer_op=conv_op, act_fn=act, dropout_prob=dropout_prob, preactivation=preact,
                             input_shape=shape[channel_dim:], batch_norm=batch_norm, layer_norm=layer_norm, instance_norm=instance_norm, group_norm=group_norm, layer_nrm_and_mean_batch_nrm=layer_nrm_and_mean_batch_nrm)
    ...
    def forward(self, inputs: TENSOR_OR_SEQ_OF_TENSORS_T) -> TENSOR_OR_SEQ_OF_TENSORS_T:
        ...
        output = (parallel_conv(in_tensor) for in_tensor, parallel_conv in zip(inputs, self.group_convolutions))
...
class MultiresolutionFusion(torch.nn.Module):
    ...
    def __init__(self, input_shapes: SIZE_OR_SEQ_OF_SIZE_T, upscale_conv_kwargs: Dict[str, Any] = None, downscale_conv_kwargs: Dict[str, Any] = None, create_new_branch: bool = True, new_branch_channels: int = None, channel_dim: int = 1, reuse_scaling_convs: bool = True, upscale_align_corners: bool = False):
        ...
        Args:
            - input_shapes: Input tensors shapes for each parallel/siamese input branches/streams, with minibatch dim or not (change `channel_dim` accordingly if needed). All `input_shapes` tensor shapes should have the same number of dimensions after `channel_dim` dimension.
            - upscale_conv_kwargs:
            ...
            - channel_dim: Channel dim index in provided `input_shapes` tensor shape(s). Dimensions after channel dim should be spatial features dims on which convolutions are applied (e.g. `input_shapes` should have two dims after `channel_dim` for 2D convoltions)
        ...
        if isinstance(input_shapes[0], NUMBER_T):
            ...
            raise ValueError('Error in `deepcv.meta.nn.MultiresolutionFusion`, `input_shapes` argument must either be a sequence of `torch.Size` with at least one input tensor shape(s) or a '
                             f'single tensor shape. {NL} NOTE: If you are performing a single conv on a single tensor shape/resolution, use `deepcv.meta.nn.layer` instead. Got `input_shapes="{input_shapes}"`')
        ...
        self.spatial_dims = len(input_shapes[0][self.channel_dim+1:])  # Assume all input features have the same spatial dims count
        ...
        self.new_branch_channels = downscale_conv_kwargs.get('out_channels', input_shapes[-1][channel_dim]) if new_branch_channels is None else new_branch_channels
        in_channels = [in_shape[channel_dim] for in_shape in input_shapes]
    ...
    def _upsample(self, x: torch.Tensor, target_shape: torch.Size, in_branch_idx: int, out_branch_idx: int) -> torch.Tensor:
        ...
        if self.reuse_scaling_convs:
            ...
            return self.upscaling_1x1_convs[out_branch_idx][in_branch_idx](x)
    ...
    def _downsample(self, x: torch.Tensor, out_channels: int, apply_n_times: int, in_branch_idx: int, out_branch_idx: int) -> torch.Tensor:
        ...
        if self.reuse_scaling_convs:
            for _i in range(apply_n_times):
                x = self.downscaling_3x3_convs[(x.shape[1], out_channels)](x)  # 1 instead of `self.channel_dim` as x have been unsqueezed if channel_dim=0
        else:
    ...
    def forward(self, inputs: TENSOR_OR_SEQ_OF_TENSORS_T) -> TENSOR_OR_SEQ_OF_TENSORS_T:
        ...
        if len(inputs) != len(self.input_shapes):
            raise ValueError(f'Error: Cant apply `{type(self).__name__}` module with {len(self.input_shapes)} input parallel branches/streams on {len(inputs)} input tensors.')
        ...
        for out_branch_idx, target_shape in enumerate(output_shapes):
            ...
            outs.append(torch.sum(scaled_features))
        return outs


class HRNetV1RepresentationHead(torch.nn.Module):
    def __init__(self, input_shapes: SIZE_OR_SEQ_OF_SIZE_T, channel_dim: int = 1) -> torch.nn.Module:
        super().__init__()
        if isinstance(input_shapes[0], NUMBER_T):
            input_shapes = [input_shapes, ]
        self.input_shapes = input_shapes
    ...
    @deepcv.meta.nn.forward_call_convention_dec(apply_parallel_forward=False, ignore_sub_refs=True)
    def _forward(self, inputs: List[torch.Tensor]) -> torch.Tensor:
        if len(inputs) != len(self.input_shapes):
            raise ValueError(f'Error: Cant apply `{type(self).__name__}` module with {len(self.input_shapes)} input parallel branches/streams on {len(inputs)} input tensors.')
...
class HRNetV2RepresentationHead(torch.nn.Module):
    ...
    def __init__(self, input_shapes: SIZE_OR_SEQ_OF_SIZE_T, submodule_params: Dict[str, Any] = None, channel_dim: int = 1, upscale_align_corners: bool = False) -> torch.nn.Module:
        """ 
        NOTE: `out_channels` must be specified in `submodule_params` (1x1 conv applied on concatenated (upscaled) representations to obtain target channel count and mix informations from all representations)
        """
        super().__init__()
        if isinstance(input_shapes[0], NUMBER_T):
            input_shapes = [input_shapes, ]
        self.input_shapes = input_shapes
        self.channel_dim = channel_dim
        max_shape_idx = np.argmax(np.prod(shape[self.channel_dim+1:] for shape in input_shapes))
        self.outout_spatial_shape = input_shapes[max_shape_idx][channel_dim+1:]
        self.outout_channels = submodule_params['out_channels']
        self.spatial_dims = len(input_shapes[0][channel_dim+1:])
        self.align_corners = upscale_align_corners

        # Define upscaling convolutions
        in_channels = sum([shape[channel_dim] for shape in input_shapes])
        submodule_params = MultiresolutionFusion._fill_conv_params(submodule_params, kernel_size=1, dims=self.spatial_dims)
        self.repr_mix_1x1_conv = conv_nd(in_channels=in_channels, **submodule_params)
    ...
    def forward(self, inputs: List[torch.Tensor]) -> torch.Tensor:
        ...
        # Upscale (interpolation only) and concatenate each upscaled representations with upper/max-resolution branch/stream input tensor. Then, apply a 1x1 conv to mix them and obtain desired channel count
        outputs = list(interpolate(inputs, out_spatial_shape=self.outout_spatial_shape, align_corners=self.align_corners))
...
class HRNetV2pRepresentationHead(HRNetV2RepresentationHead):
    def __init__(self, input_shapes: SIZE_OR_SEQ_OF_SIZE_T, repr_mix_conv_kwargs: Dict[str, Any] = None, downscale_conv_kwargs: Dict[str, Any] = None, max_downscaling_count: int = None, channel_dim: int = 1) -> torch.nn.Module:
        """
        ...
        """
        super().__init__(input_shapes=input_shapes, submodule_params=repr_mix_conv_kwargs, channel_dim=channel_dim)
        self.downscaling_count = len(input_shapes) - 1 if max_downscaling_count is None else max_downscaling_count
        downscale_conv_kwargs = self._fill_conv_params(downscale_conv_kwargs, kernel_size=3, stride=2, padding_mode='zero', dims=self.spatial_dims)
...
def hrnet_input_stem(input_shape: torch.Size, submodule_params: Dict[str, Tuple[Sequence[Any], Any]], conv_count: int = 2, act_fn: Union[Sequence[Optional[Type]], Optional[Type]] = torch.nn.ReLU,
                     preactivation: Union[Sequence[bool], bool] = False, dropout_prob: float = None,
                     batch_norm=None, layer_norm=None, instance_norm=None, group_norm=None, layer_nrm_and_mean_batch_nrm=None, channel_dim: int = 1) -> torch.nn.Module:
    ...
    .. See `deepcv.meta.nn.layer` for more details on `act_fn`, `preactivation`, `dropout_prob`, `batch_norm`, `layer_norm`, `instance_norm`, `group_norm``and `layer_nrm_and_mean_batch_nrm` arguments.
    """
    ...
    sequential_subm = torch.nn.Sequential(OrderedDict(strided_convs))
