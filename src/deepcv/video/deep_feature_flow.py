...
def deep_feature_flow_inference(video_frames: Iterable[torch.Tensor], features_model: torch.nn.Module, tasks_models: Sequence[torch.nn.Module], flow_estimation: torch.nn.Module, keyframe_period: int) -> Generator[Dict[int, Tuple], None, None]:
    ...
    def _propagation_W(keyframe_features: torch.Tensor, flow_field_2d: torch.Tensor, scale_field: torch.Tensor) -> torch.Tensor:
    ...
    for i, frame in enumerate(video_frames):
        if (i + 1) % keyframe_period == 0:
            ...
            flow_field_2d = flow_estimation(frame, keyframe)
            feature_maps = _propagation_W(keyframe_features, flow_field_2d, scale_field)
